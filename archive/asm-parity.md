...
| `$-.` | asm→**forth** | **forth** | forth/forth | 5 | **done** — shared Forth in ff2.boot, asm removed from ff.asm |
...
| `notfound` | asm | — → **asm** | asm/asm | 7 | **done** — VECT with default `call _error / "???"` |
| `number` | asm | — → **asm** | asm/asm | 7 | **done** — WORD64 exposing _number |
| `litcomp` | asm | — → **asm** | asm/asm | 7 | **done** — _literalcompiler extracted as callable subroutine |
| `number.` | asm | — → **asm** | asm/asm | 7 | **done** — Forth-callable wrapper with explicit base |
| `classes` | asm | — → **asm** | asm/asm | 7 | **done** — _classes table + _icall/_ilit/_ccerr handlers |
| `>SC` | asm | — | TBD | 7 | **kept** — structural diff in SWAPbit handling |
| `>S1` | asm | — | TBD | 7 | **kept** — structural diff in SWAPbit handling |
...
to become Forth. These stay assembly-only on both
arches. Revisit if self-hosting ever seems worthwhile.

| Word | i386 | x64 | Status |
|------|------|-----|--------|
| `:`  | asm  | asm | **kept** — stay asm |
| `;`  | asm  | asm | **kept** — stay asm |
| `anon` | asm | asm | **kept** — stay asm |
| `anon:` | asm | asm | **kept** — stay asm |
...
- **;;` + tailrec**: DG reversed ;;` target: keep in assembly on both
  ...
  ff2.boot. Tier 4 compiler words (: ; anon anon:) kept — stay
