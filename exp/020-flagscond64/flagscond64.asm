...
;;
;; In FreeForth, comparison words do NOT produce boolean values on the
;; stack. Instead, they:
;;   1. Emit a CMP or TEST instruction (setting CPU FLAGS)
;;   2. Store the conditional jump opcode in the cond_jmp variable
;; Then IF/UNTIL/WHILE read cond_jmp and emit the conditional jump.
;;
;; This eliminates the setcc+movzx+neg+DROP sequence and preserves
;; the data stack across comparisons. All stack operations use LEA
;; instead of ADD/SUB to preserve FLAGS between comparison and IF.
;; =====================================================================

;; 0- ( -- ): emit test TOS,TOS to set FLAGS without modifying stack
;; Used before unary conditions (0=, 0<, 0<>, 0>, 0<=, 0>=)
;; Runtime effect: CPU FLAGS reflect the value of TOS
_0minus_inline:
        mov byte [rbp], $48
        mov word [rbp+1], $DB85     ; test rbx, rbx
        add rbp, 3
        jmp _s09                    ; SWAPbit: test rdx,rdx if swapped

;; Binary flags-based comparisons: emit cmp NOS,TOS and store condition.
;; Stack is UNCHANGED — the cmp only sets FLAGS.

;; < ( -- ): emit cmp NOS,TOS, store jl condition
_lt_flags:
        mov byte [cond_jmp], $7C    ; jl
        jmp _emit_cmp_s

;; > ( -- ): emit cmp NOS,TOS, store jg condition
_gt_flags:
        mov byte [cond_jmp], $7F    ; jg
        jmp _emit_cmp_s

;; = ( -- ): emit cmp NOS,TOS, store je condition
_eq_flags:
        mov byte [cond_jmp], $74    ; je
        jmp _emit_cmp_s

;; <> ( -- ): emit cmp NOS,TOS, store jne condition
_neq_flags:
        mov byte [cond_jmp], $75    ; jne
        jmp _emit_cmp_s

;; <= ( -- ): emit cmp NOS,TOS, store jle condition
_le_flags:
        mov byte [cond_jmp], $7E    ; jle
        jmp _emit_cmp_s

;; >= ( -- ): emit cmp NOS,TOS, store jge condition
_ge_flags:
        mov byte [cond_jmp], $7D    ; jge
        jmp _emit_cmp_s
...
;; Unary flags-based conditions: just store the condition in cond_jmp.
;; Expect a preceding 0- (or any FLAGS-setting instruction).

;; 0= ( -- ): store je condition (jump if ZF set)
_zeq_flags:
        mov byte [cond_jmp], $74
        ret

;; 0<> ( -- ): store jne condition (jump if ZF clear)
_zneq_flags:
        mov byte [cond_jmp], $75
        ret

;; 0< ( -- ): store jl condition (jump if negative)
_zlt_flags:
        mov byte [cond_jmp], $7C
        ret

;; 0> ( -- ): store jg condition (jump if positive, non-zero)
_zgt_flags:
        mov byte [cond_jmp], $7F
        ret

;; 0<= ( -- ): store jle condition
_zle_flags:
        mov byte [cond_jmp], $7E
        ret

;; 0>= ( -- ): store jge condition (jump if non-negative)
_zge_flags:
...
;; These are the FreeForth "dotted" versions (e.g., <. >. =.)
;; for when you actually need a boolean (-1 or 0) on the data stack.
;; =====================================================================

;; =. ( a b -- flag ): boolean equality
_eq_inline:
        mov byte [rbp], $48
        mov word [rbp+1], $DA39     ; cmp rdx, rbx
        add rbp, 3
...
;; Calls _rst first to sync SWAPbit (xchg doesn't affect FLAGS).
_if:
        call _rst
        ;; Check if a condition was set
        movzx eax, byte [cond_jmp]
        mov byte [cond_jmp], 0      ; clear it
        test al, al
        jz .fallback
        ;; FLAGS-based path: emit long conditional jump
        ;; Invert condition: XOR 1 (e.g., jl $7C → jge $7D)
        xor al, 1
        ;; Emit: 0F (al+$10) rel32  — long conditional jump
        mov byte [rbp], $0F
        add al, $10
        mov byte [rbp+1], al
        add rbp, 2
        jmp .push_patch
.fallback:
        ;; No condition set — traditional boolean test (backward compat)
        ;; Compile: test rbx, rbx (48 85 DB)
        mov byte [rbp], $48
        mov word [rbp+1], $DB85
        add rbp, 3
        ;; Compile: DROP1 (flags-preserving)
...
_until:
        ...
        test al, al
        jz .fallback
        ;; FLAGS-based path: emit long conditional backward jump
        ;; Invert condition (UNTIL loops while condition is FALSE)
        xor al, 1
        mov byte [rbp], $0F
        add al, $10
        mov byte [rbp+1], al
        ...
        jmp .calc_offset
.fallback:
        ;; Traditional: test rbx, rbx + DROP1 + jz
        mov byte [rbp], $48
        mov word [rbp+1], $DB85
        add rbp, 3
        ;; DROP1 (flags-preserving)
        mov byte [rbp], $48
        mov word [rbp+1], $D389
        add rbp, 3
        mov byte [rbp], $49
...
;; FLAGS-based comparison words (ct=2): emit cmp/test, store condition
...
;; DOTTED comparisons (ct=2): produce boolean values on the stack
