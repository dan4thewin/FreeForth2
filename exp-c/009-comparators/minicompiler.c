/*
 * Portable FreeForth2 — Experiment 009: Comparators and Selectors
 *
 * Key innovation: SACRIFICIAL COMPARE pattern.
 *
 * Every ALU primitive is compiled in TWO versions:
 *   plain:     void alu_sub(void)    { tos = nos - tos; }
 *   sacrifice: int64_t alu_sub_s(void) { tos = nos - tos; return tos == 0; }
 *
 * On ARM64, the sacrifice coerces GCC to emit SUBS instead of SUB
 * (fusing the subtraction and flag-setting into one instruction).
 * On x86-64, SUB already sets flags, so the plain version suffices.
 *
 * Self-calibrating extraction: we compile both, compare lengths,
 * and auto-select the right bytes.  Architecture-specific knowledge
 * is limited to register assignment, stack plumbing, and branch
 * encoding — the ALU extraction is universal.
 *
 * Full comparator × selector matrix:
 *   Comparators (flag-setting):  0-, +, -, 1-, 1+, &, |, ^, negate, cmp
 *   Selectors (compile-time):    0= (ZF), 0<> (!ZF), 0< (SF), 0> (GT)
 *   Flow words:                  IF/THEN, BEGIN/UNTIL
 */
...
static int init(void)
{
	...
	/* ---- Extract macros ---- */
	if (discover_macros(macro_table, mac, MAC_COUNT) != 0)
		return -1;
